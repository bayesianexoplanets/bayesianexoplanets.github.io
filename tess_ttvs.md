...
Click a row to see the folded and per-epoch stacked transit (left) next to the O−C measurements with the two-planet model overlaid (top right) — solid line the posterior median, shaded the 68% band, which is wide where several distinct solutions fit comparably well — and the inferred mass and period of the perturbing planet (bottom right). There the filled contours are the neural posterior; where an independent N-body MCMC chain was run for the system, its posterior is overlaid as unfilled contours. Contours enclose 39% and 87% of the probability.
...
| TTV SNR | Matched-filter signal-to-noise of the O−C data against the median physical two-planet model |
