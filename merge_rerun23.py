"""merge_rerun23.py -- merge the Rerun23 "fit at fixed values" results into tois.csv.

For the 23 rerun systems, replace the pipeline-recomputed columns of TESS corrected/tois.csv with
the values from Rerun23/known_candidates/{tic}.csv (matched by TOI), refresh the per-planet website
plot, and recompute log10(p) (from the kept per-star Singh-Maddala sm_sf_grid) + passed/failed tests.

Special cases:
  * 364395234 (TOI 1232.01 & new 1232.02): the contaminated null is DELETED -- blank μ/σ(SNR|null),
    nst_samples, sm_sf_grid, log10(p value); the planet then appears "without a null run". 1232.02 is
    placed adjacent to 1232.01. passed/failed still come from the direct diagnostics.
  * everyone else: log10(p) recomputed from the kept sm_sf_grid; null columns untouched.
...
NULL_BLANK_TICS = {364395234}                       # delete contaminated null -> "no null run"
NULL_COLS = ["μ(SNR | null)", "σ(SNR | null)", "nst_samples", "sm_sf_grid", "log10(p value)"]
...
def main(apply):
    ...
    print(f"website rows={len(w)} | Rerun23 TICs={len(tics)}")
    ...
    for tic in tics:
        ...
        for known_idx, (wi, wrow) in enumerate(wsub.iterrows()):
            ...
            if tic in NULL_BLANK_TICS:
                for c in NULL_COLS: upd[c] = np.nan          # delete contaminated null
            else:
            ...
            if apply:
                ...
                extra = " [NULL DELETED]" if tic in NULL_BLANK_TICS else ""
                print(f"  TIC {tic} TOI {k} (idx{known_idx}, plot {int(r['plot_index'])}): "
                      f"P {wrow['Period']:.4f}->{upd['Period']:.4f}  SNR {wrow['SNR']:.2f}->{snr:.2f}  "
                      f"ntr {wrow['Number of Valid Transits']}->{ntr}  pass {wrow['passed_all_tests']}->{passed}"
                      f" [{upd['failed_tests']}]{extra}")
            n_rows += 1
    ...
    # place the appended 1232.02 row immediately after 1232.01 (contiguous per-TIC grouping),
    # WITHOUT reordering any other rows (a global sort would scramble per-TIC plot indices).
    if apply:
        idx = list(w.index)
        is01 = (w["TIC"] == 364395234) & (w["TOI"].map(toi_key) == "1232.01")
        is02 = (w["TIC"] == 364395234) & (w["TOI"].map(toi_key) == "1232.02")
        if is01.any() and is02.any():
            i01 = w.index[is01][0]; i02 = w.index[is02][0]
            if idx.index(i02) != idx.index(i01) + 1:
                idx.remove(i02); idx.insert(idx.index(i01) + 1, i02)
                w = w.loc[idx].reset_index(drop=True)
